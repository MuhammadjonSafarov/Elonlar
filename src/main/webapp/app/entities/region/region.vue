<template>
  <div>
    <h2 id="page-heading" data-cy="RegionHeading">
      <span v-text="$t('elonlarApp.region.home.title')" id="region-heading">Regions</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('elonlarApp.region.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'RegionCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('elonlarApp.region.home.createLabel')"> Create a new Region </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && regions && regions.length === 0">
      <span v-text="$t('elonlarApp.region.home.notFound')">No regions found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="regions">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('nameUzLat')">
              <span v-text="$t('elonlarApp.region.nameUzLat')">Name Uz Lat</span>
            ...
            <th scope="row" v-on:click="changeOrder('nameUzKr')">
              <span v-text="$t('elonlarApp.region.nameUzKr')">Name Uz Kr</span>
            ...
            <th scope="row" v-on:click="changeOrder('nameRu')">
              <span v-text="$t('elonlarApp.region.nameRu')">Name Ru</span>
            ...
            <th scope="row" v-on:click="changeOrder('nameEn')">
              <span v-text="$t('elonlarApp.region.nameEn')">Name En</span>
            ...
            <th scope="row" v-on:click="changeOrder('longitude')">
              <span v-text="$t('elonlarApp.region.longitude')">Longitude</span>
            ...
            <th scope="row" v-on:click="changeOrder('latitude')">
              <span v-text="$t('elonlarApp.region.latitude')">Latitude</span>
        ...
        <tbody>
          <tr v-for="region in regions" :key="region.id" data-cy="entityTable">
            ...
            <td>{{ region.nameUzLat }}</td>
            <td>{{ region.nameUzKr }}</td>
            <td>{{ region.nameRu }}</td>
            <td>{{ region.nameEn }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="elonlarApp.region.delete.question" data-cy="regionDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-region-heading" v-text="$t('elonlarApp.region.delete.question', { id: removeId })">
