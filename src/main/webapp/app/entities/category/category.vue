<template>
  <div>
    <h2 id="page-heading" data-cy="CategoryHeading">
      <span v-text="$t('elonlarApp.category.home.title')" id="category-heading">Categories</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('elonlarApp.category.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CategoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('elonlarApp.category.home.createLabel')"> Create a new Category </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && categories && categories.length === 0">
      <span v-text="$t('elonlarApp.category.home.notFound')">No categories found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="categories">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('nameUzLat')">
              <span v-text="$t('elonlarApp.category.nameUzLat')">Name Uz Lat</span>
            ...
            <th scope="row" v-on:click="changeOrder('nameUzKr')">
              <span v-text="$t('elonlarApp.category.nameUzKr')">Name Uz Kr</span>
            ...
            <th scope="row" v-on:click="changeOrder('nameRu')">
              <span v-text="$t('elonlarApp.category.nameRu')">Name Ru</span>
            ...
            <th scope="row" v-on:click="changeOrder('nameEn')">
              <span v-text="$t('elonlarApp.category.nameEn')">Name En</span>
            ...
            <th scope="row" v-on:click="changeOrder('icon')">
              <span v-text="$t('elonlarApp.category.icon')">Icon</span>
        ...
        <tbody>
          <tr v-for="category in categories" :key="category.id" data-cy="entityTable">
            ...
            <td>{{ category.nameUzLat }}</td>
            <td>{{ category.nameUzKr }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="elonlarApp.category.delete.question" data-cy="categoryDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-category-heading" v-text="$t('elonlarApp.category.delete.question', { id: removeId })">
