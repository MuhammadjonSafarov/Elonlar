<template>
  <div>
    <h2 id="page-heading" data-cy="DistrictHeading">
      <span v-text="$t('elonlarApp.district.home.title')" id="district-heading">Districts</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('elonlarApp.district.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'DistrictCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('elonlarApp.district.home.createLabel')"> Create a new District </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && districts && districts.length === 0">
      <span v-text="$t('elonlarApp.district.home.notFound')">No districts found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="districts">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('nameUzLat')">
              <span v-text="$t('elonlarApp.district.nameUzLat')">Name Uz Lat</span>
            ...
            <th scope="row" v-on:click="changeOrder('nameUzKr')">
              <span v-text="$t('elonlarApp.district.nameUzKr')">Name Uz Kr</span>
            ...
            <th scope="row" v-on:click="changeOrder('nameRu')">
              <span v-text="$t('elonlarApp.district.nameRu')">Name Ru</span>
            ...
            <th scope="row" v-on:click="changeOrder('nameEn')">
              <span v-text="$t('elonlarApp.district.nameEn')">Name En</span>
            ...
            <th scope="row" v-on:click="changeOrder('longitude')">
              <span v-text="$t('elonlarApp.district.longitude')">Longitude</span>
            ...
            <th scope="row" v-on:click="changeOrder('latitude')">
              <span v-text="$t('elonlarApp.district.latitude')">Latitude</span>
            ...
            <th scope="row" v-on:click="changeOrder('region.id')">
              <span v-text="$t('elonlarApp.district.region')">Region</span>
        ...
        <tbody>
          <tr v-for="district in districts" :key="district.id" data-cy="entityTable">
            ...
            <td>{{ district.nameUzLat }}</td>
            <td>{{ district.nameUzKr }}</td>
            <td>{{ district.nameRu }}</td>
            <td>{{ district.nameEn }}</td>
            <td>{{ district.longitude }}</td>
            <td>{{ district.latitude }}</td>
            <td>
              <div v-if="district.region">
                <router-link :to="{ name: 'RegionView', params: { regionId: district.region.id } }">{{ district.region.id }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="elonlarApp.district.delete.question" data-cy="districtDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-district-heading" v-text="$t('elonlarApp.district.delete.question', { id: removeId })">
